...
dfn <- data.frame("Non-current financial asset" = c("Currency options", "Forward exchange contracts", "Revaluation of total return swap",
                                                    "Security deposits", "Options", "Earn-out components", "Promissory notes", "Investment in FC Bayern München AG",
                                                    "Other equity investments", "Other investments", "Loans"),
                  "Value" = c(10, 2, 59, 110, 86, 36, 149, 84, 81, 201, 1))
...
kolacnik_kfn <- ggplot(kfn, aes(x = "", y = Value, fill = Current.financial.asset)) + 
  ...
  labs(title ="Current financial assets", fill = "Current financial asset") +
...
kolacnik_dfn <- ggplot(dfn, aes(x = "", y = Value, fill = Non.current.financial.asset)) + 
  geom_bar(stat = "identity", color = "white") +
  labs(title ="Non current financial assets", fill = "Non current financial asset") +
  coord_polar("y") + theme_void() + scale_fill_brewer(palette="Set3")
...
kolacnik_invest <- ggplot(by_investitor, aes(x = "", y = share, fill = investitor)) + 
  geom_bar(stat = "identity", color = "white") +
  labs(title ="Shareholder structure by investor group") +
  coord_polar("y") + theme_void() + scale_fill_brewer(palette="Set3")
...
graf_dividende <- plot(dividende[, c(1, 3)], main = "Dividend payment", col = "lightskyblue", type = "l", ylim = c(26, 1380), ylab = "mio $", lwd = 2)
